...
constexpr uint16_t kLimb = 0x8C31;             // darker limb
...
{
    int count = 1 + score_ / 10;
    if (count > kWaveMaxCount) {
        count = kWaveMaxCount;
    }
    // First-wave courtesy (score==0): never drop a spike directly on the
    // player's spawn path before the player has a chance to react. From the
    // first dodge onward spikes fall anywhere; static play dies fast.
    const bool courtesy = score_ == 0;
    ...
    for (int attempt = 0; attempt < count; ++attempt) {
        ...
        // In-wave spikes keep a minimum separation; with first-wave courtesy
        // they also never spawn directly above the player. Retry with a
        // jitter up to 8 times.
        bool spaced = false;
        for (int retry = 0; retry < 8; ++retry) {
            spaced = true;
            if (courtesy && std::fabs(x - player_x_) < kWaveSpacingMin) {
                spaced = false;
            }
            ...
            x = clamp_float(x + (rand_float() - 0.5f) * 90.0f,
                            24.0f, kFWidth - 24.0f);
        }
        if (!spaced) {
            continue;  // no room this wave
        }
        ...
        s.x = x;
        s.y = kSpikeSpawnY;
        float vy = kSpikeVInitial + static_cast<float>(score_) * kSpikeVRamp;
        vy += rand_float() * kSpikeVRandom;
        s.vy = vy > kSpikeVMax ? kSpikeVMax : vy;
...
void RagdollAvalancheApp::step_substep()
{
    ...
    // --- Player steering (slippery, like the original arrow-key feel) ---
    ...
    if (!dead) {
        ...
        if (valid) {
            ...
            player_vx_ = (player_vx_ + ax * kSubstepDt) * kDamping;
            player_vy_ = (player_vy_ + ay * kSubstepDt) * kDamping;
            const float speed = std::sqrt(player_vx_ * player_vx_ +
                                          player_vy_ * player_vy_);
            if (speed > kMaxSpeed) {
                player_vx_ = player_vx_ * (kMaxSpeed / speed);
                player_vy_ = player_vy_ * (kMaxSpeed / speed);
            }
        } else {
            player_vx_ *= kDamping;
            player_vy_ *= kDamping;
        ...
        player_x_ = clamp_float(player_x_ + player_vx_ * kSubstepDt,
                                kPlayerMinX, kPlayerMaxX);
        player_y_ = clamp_float(player_y_ + player_vy_ * kSubstepDt,
                                kPlayerMinY, kPlayerMaxY);
    }
    ...
    if (!dead) {
        ...
        if (wave_timer_ >= wave_interval_) {
            ...
            const float ramp =
                ...
                static_cast<float>(score_) * kWaveIntervalRamp;
            wave_interval_ =
                ramp < kWaveIntervalMin ? kWaveIntervalMin : ramp;
        }
    }
    ...
    // Pass 2: collision with the player (one kill per substep at most).
    if (!was_dead) {
        for (int i = 0; i < kMaxSpikes; ++i) {
            ...
            const float dx = player_x_ - s.x;
            const float dy = player_y_ - s.y;
            if (dx * dx + dy * dy <
                kSpikeHitRadius * kSpikeHitRadius) {
                kill_player();
                break;
            }
        }
...
void RagdollAvalancheApp::fill_snapshot(AvalancheFrame &snapshot)
{
    ...
    snapshot.player_vx = player_vx_;
    // Velocity lean clamped for the renderer's rotation value.
    float lean = player_vx_ * 0.002f;
    lean = clamp_float(lean, -0.9f, 0.9f);
    snapshot.player_rot = lean;
                                     ...
                                     uint16_t shaft_color, uint16_t tip_color)
{
    ...
    constexpr int kHalfTop = 3;
    // Tapered shaft: each row narrows toward the tip.
    for (int row = 0; row < kLength; ++row) {
        const int hw = kHalfTop - (kHalfTop * row) / kLength;
        if (hw <= 0) {
            continue;
        }
        draw_rect(pixels, width, y0, rows,
                  cx - hw, tip - row, cx + hw, tip - row + 1, shaft_color);
    }
    // Bright tip point.
    draw_disc(pixels, width, y0, rows, cx, tip, 2, tip_color);
}

void RagdollAvalancheApp::draw_ragdoll(uint16_t *pixels, int width, int y0,
                                       int rows, float x, float y,
                                       float lean, uint32_t tick,
                                       uint16_t body_color,
                                       uint16_t head_color)
{
    // Stick-figure ragdoll on local axes: u = body direction (head-up at
    // lean=0, tilts with lean), p = perpendicular. Limbs flail from the
    // animation clock. lean=1.57 flops the figure horizontal (death pose).
    const float ux = std::sin(lean);
    const float uy = std::cos(lean);
    const float px = std::cos(lean);
    const float py = -std::sin(lean);

    const int ix = static_cast<int>(x + 0.5f);
    const int iy = static_cast<int>(y + 0.5f);
    const int head_r = static_cast<int>(kHeadRadius + 0.5f);
    const int limb_r = 1;

    // Torso: from origin up 7 units (the figure hangs from the head).
    const int bx0 = ix;
    const int by0 = iy;
    const int bx1 = ix + static_cast<int>(ux * 7.0f + 0.5f);
    const int by1 = iy + static_cast<int>(uy * 7.0f + 0.5f);

    // Head.
    const int hx = ix - static_cast<int>(ux * 7.0f + 0.5f);
    const int hy = iy - static_cast<int>(uy * 7.0f + 0.5f);
    draw_disc(pixels, width, y0, rows, hx, hy, head_r, head_color);

    // Body.
    draw_segment(pixels, width, y0, rows, bx0, by0, bx1, by1, 2, body_color);

    // Shoulder/hip anchors.
    const int sx = bx0 + static_cast<int>(ux * 1.0f + 0.5f);
    const int sy = by0 + static_cast<int>(uy * 1.0f + 0.5f);
    const int hpx = bx1;
    const int hpy = by1;

    const float flap = 0.55f;
    const float a1 = std::sin(static_cast<float>(tick) * 0.30f) * flap;
    const float a2 = std::sin(static_cast<float>(tick) * 0.30f + 2.1f) * flap;

    // Arms from the shoulder.
    for (int arm = 0; arm < 2; ++arm) {
        const float a = arm == 0 ? a1 : a2;
        const float dir_x = px * std::sin(a) + ux * std::cos(a);
        const float dir_y = py * std::sin(a) + uy * std::cos(a);
        draw_segment(pixels, width, y0, rows, sx, sy,
                     sx + static_cast<int>(dir_x * 6.0f + 0.5f),
                     sy + static_cast<int>(dir_y * 6.0f + 0.5f),
                     limb_r, kLimb);
    }

    // Legs from the hip.
    for (int leg = 0; leg < 2; ++leg) {
        const float a = leg == 0 ? -a1 : -a2;
        const float dir_x = px * std::sin(a) + ux * std::cos(a);
        const float dir_y = py * std::sin(a) + uy * std::cos(a);
        draw_segment(pixels, width, y0, rows, hpx, hpy,
                     hpx + static_cast<int>(dir_x * 6.0f + 0.5f),
                     hpy + static_cast<int>(dir_y * 6.0f + 0.5f),
                     limb_r, kLimb);
    }
}
                                        ...
                                        int y0, int rows)
{
    ...
    // Player ragdoll: signed lean (head trails motion), flopped horizontal
    // (lean = pi/2) after death.
    const float lean = frame.game_over ? 1.57f : frame.player_rot;
    draw_ragdoll(pixels, width, y0, rows,
                 frame.player_x, frame.player_y, lean, frame.tick,
                 kBody, kHead);
